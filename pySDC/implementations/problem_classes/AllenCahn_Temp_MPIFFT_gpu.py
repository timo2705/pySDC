    ...
    def u_exact(self, t):
        ...
        def circle():
            ...
            if self.params.spectral:
                ...
                tmp_me[:] = 0.5 * (1.0 + cp.tanh((self.params.radius - cp.sqrt(r2)) / (cp.sqrt(2) * self.params.eps)))
            return tmp_me
        ...
        def sine():
            tmp_me = newDistArray(self.fft, self.params.spectral)
            if self.params.spectral:
                tmp = 0.5 * (2.0 + 0.0 * cp.sin(2 * cp.pi * self.X[0]) * cp.sin(2 * cp.pi * self.X[1]))
                tmp_me[:] = self.fft.forward(tmp)
            else:
                tmp_me[:] = 0.5 * (2.0 + 0.0 * cp.sin(2 * np.pi * self.X[0]) * cp.sin(2 * np.pi * self.X[1]))
            return tmp_me
